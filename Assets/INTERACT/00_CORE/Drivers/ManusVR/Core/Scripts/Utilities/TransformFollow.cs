...
namespace ManusVR.Core.Utilities
{
	public class TransformFollow : MonoBehaviour
	{
		...
		protected void FollowPosition()
		{
			...
			{
				float alpha =
					Mathf.Clamp01(Vector3.Distance(targetPosition, targetTransformPosition) / maxDistanceDeltaPerFrame);
		...
		protected void FollowRotation()
		{
			...
			if (shouldSmoothPosition)
			{
				float alpha = Mathf.Clamp01(Quaternion.Angle(Quaternion.Euler(targetRotation), targetTransformRotation) /
				                            maxDistanceDeltaPerFrame);
